...
<head>
    ...
    <style>
        ...
        .watermark { position: absolute; bottom: 30px; width: 100%; text-align: center; opacity: 0.4; font-size: 11px; font-weight: 900; letter-spacing: 0.15em; text-transform: uppercase; pointer-events: none; z-index: 0; }
    ...
    <script type="text/babel">
        ...
        function App() {
            ...
            const sendMessage = async () => {
                ...
                } catch (err) { setMessages(prev => [...prev, { role: 'assistant', content: "Error: Session interrupted." }]); } finally { setLoading(false); }
            };
                ...
                return (
                    <div className={`relative flex items-center justify-center h-full p-6 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-900'}`}>
                        <div className={`w-full max-w-sm p-10 rounded-[3rem] shadow-2xl text-center border z-10 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                            ...
                            <p className="opacity-80 text-[10px] font-bold uppercase tracking-[0.3em] mb-8">Clinical Console</p>
                            <div className="mb-8">
                                <a href="https://console.groq.com/keys" target="_blank" rel="noreferrer" className="bg-red-500/10 text-red-500 p-3 rounded-xl text-[10px] font-black uppercase tracking-widest border border-red-500/20 block hover:bg-red-500/20 transition-all">Get API Key From Groq</a>
                            </div>
                            <input type="password" placeholder="Enter clinical key" className={`w-full p-4 rounded-2xl mb-4 text-center text-sm font-bold border transition-all ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-200'}`} value={apiKey} onChange={(e) => setApiKey(e.target.value)} />
                            <button onClick={() => apiKey.startsWith('gsk_') ? setView('chat') : alert('Invalid Key')} className="w-full bg-red-600 p-4 rounded-2xl font-black uppercase text-white shadow-lg active:scale-95 transition-all">Start Session</button>
                        </div>
                        <div className={`watermark ${isDarkMode ? 'text-red-400' : 'text-slate-400'}`}>Powered by Gemini & Groq</div>
                    </div>
                );
            }
                ...
                <div className={`flex flex-col h-full ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
                    ...
                    {showSettings && (
                        <div className={`absolute top-16 right-4 w-64 rounded-[2rem] shadow-2xl p-5 z-40 border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                             <p className="text-[10px] font-black uppercase opacity-40 mb-3 tracking-[0.2em]">Diagnostic Models</p>
                             ...
                             <p className="text-[10px] font-black uppercase opacity-40 mb-3 tracking-[0.2em]">Interface</p>
                             <button onClick={() => setIsDarkMode(!isDarkMode)} className="w-full p-3 rounded-xl mb-4 text-xs font-bold bg-red-600 text-white shadow-sm">Toggle Theme</button>
                             <button onClick={() => setView('login')} className="w-full text-red-500 text-[10px] font-black uppercase tracking-widest text-center">Logout</button>
                        </div>
                    )}
                    ...
                    <footer className="fixed bottom-0 left-0 right-0 p-4 border-t pb-8 bg-inherit">
                        <div className="flex gap-2 max-w-3xl mx-auto">
                            <input className={`flex-1 p-4 rounded-2xl text-sm border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-100 border-slate-200'}`} placeholder="Symptoms..." value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendMessage()} />
                            <button onClick={sendMessage} className="bg-red-600 p-4 rounded-2xl shadow-lg px-6 active:scale-90 transition-all"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg></button>
                        </div>
                    </footer>
                </div>
            );
